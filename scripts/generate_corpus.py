...
async def run_edge(spec: dict) -> list[dict]:
    ...
    for syl in spec["syllables"]:
        ...
        for tone_str, char in syl["tones"].items():
            tone = int(tone_str)
            text = char  # character is the highest-fidelity input
            for voice_id, edge_voice in EDGE_VOICES.items():
                out = OUT_DIR / f"{pinyin}-{tone}-{voice_id}.mp3"
                print(f"[edge] {pinyin}{tone} ({char}) → {voice_id}")
...
def run_cosyvoice(spec: dict) -> list[dict]:
    try:
    ...
    for syl in spec["syllables"]:
        ...
        for tone_str, char in syl["tones"].items():
            ...
            for vid, (prompt_audio, prompt_text) in prompts.items():
                ...
                print(f"[cosy] {pinyin}{tone} ({char}/{mark(pinyin, tone)}) → {vid}")
...
def main() -> None:
    ...
    if args.backend == "edge":
        items = asyncio.run(run_edge(spec))
    else:
        items = run_cosyvoice(spec)
